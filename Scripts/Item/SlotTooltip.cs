...
public class SlotTooltip : UI_PopUp
{
    ...
    private void Awake()
    {
        //_goBase = transform.GetChild(0).gameObject;
    ...
    private void HideTooltipProperty(Item _item)
    {
        if (_item.itemType == ItemType.Consumable)
        {
            switch (_item.ammoType)
    ...
    // 각 아이템의 속성 값 끄기
    public void CloseTooltipProperty(Item _item)
    {
        HideTooltipProperty(_item);
    }
}
