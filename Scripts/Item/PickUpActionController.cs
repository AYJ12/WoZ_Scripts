...
public class PickUpActionController : MonoBehaviour
{
    ...
    private void TryAction()
    {
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            if (!(_itemPickUp.item.itemType == ItemType.Consumable))
            {
                // 획득 메서드
                PickUpItem();
            }
            else
            {
                ...
                switch (_itemPickUp.item.ammoType)
                {
                    case AmmoType.Total:
                        PickUpItem();
                        break;
    ...
    private void PickUpItem()
    {
        if (_itemPickUp)
        {
            _itemInventoryController.SetItemData(_itemPickUp.item);
    ...
    {
        if (_itemPickUp.item.ammoType == AmmoType.Rifle || _itemPickUp.item.ammoType == AmmoType.Sniper)
        {
            _itemInventoryController.SetItemData(_itemPickUp.item);
            ItemUseAppear();
        }
    ...
    {
        // 획득 안내 문구 활성화 호출
        _showText.InfoActionText(_itemPickUp.item);
    ...
    {
        // 획득 문구 활성화 호출
        _showText.PickUpActionText(_itemPickUp.item);
    ...
    {
        // 사용 문구 활성화 호출
        if (_itemPickUp.item.ammoType == AmmoType.Total)
        {
            _showText.UseActionText(_itemPickUp.item);
